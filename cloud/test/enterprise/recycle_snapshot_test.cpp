...
int main(int argc, char** argv) {
    ...
    if (!cloud::init_glog("enterprise_recycle_snapshot_test")) {
...
// A test case to recycle the timeouted prepare snapshot.
TEST(RecycleSnapshotTest, RecycleTimeoutSnapshot) {
    auto meta_service = get_meta_service();
    auto txn_kv = meta_service->txn_kv();
    std::string instance_id = "recycle_snapshot_test_abort_snapshot_instance";
    ...
    // begin a snapshot
    SnapshotContext ctx;
    begin_snapshot(meta_service.get(), cloud_unique_id, "auto-gen", &ctx);
    ...
    ASSERT_EQ(snapshots[0].status(), SnapshotStatus::SNAPSHOT_ABORTED);
}

// A test case to recycle the aborted snapshot.
TEST(RecycleSnapshotTest, RecycleAbortedSnapshot) {
    auto meta_service = get_meta_service();
    auto txn_kv = meta_service->txn_kv();
    std::string instance_id = "recycle_snapshot_test_abort_snapshot_instance";
    std::string cloud_unique_id = fmt::format("1:{}:0", instance_id);
    ...
    // begin a snapshot and abort it
    SnapshotContext ctx;
    begin_snapshot(meta_service.get(), cloud_unique_id, "auto-gen", &ctx);
    abort_snapshot(meta_service.get(), cloud_unique_id, ctx.snapshot_id, "test case abort");
    ...
    // The aborted snapshot should be recycled.
    get_snapshots(meta_service.get(), instance_id, snapshots);
    ASSERT_EQ(snapshots.size(), 0);
}

// A test case to recycle the normal snapshot to keep the max reserved snapshots.
TEST(RecycleSnapshotTest, RecycleNormalSnapshot) {
    auto meta_service = get_meta_service();
    auto txn_kv = meta_service->txn_kv();
    std::string instance_id = "recycle_snapshot_test_normal_snapshot_instance";
    std::string cloud_unique_id = fmt::format("1:{}:0", instance_id);
    MOCK_GET_INSTANCE_ID(instance_id);
    create_and_refresh_instance(meta_service.get(), instance_id);
    int64_t db_id = 1, table_id = 2, index_id = 3, partition_id = 4, tablet_id = 5;
    ...
    get_snapshots(meta_service.get(), instance_id, snapshots);
    ASSERT_EQ(snapshots.size(), 1);
    ASSERT_EQ(snapshots[0].status(), SnapshotStatus::SNAPSHOT_NORMAL);
    ASSERT_EQ(snapshots[0].journal_id(), 100);

    begin_snapshot(meta_service.get(), cloud_unique_id, "auto-gen", &ctx);
    commit_snapshot(meta_service.get(), cloud_unique_id, ctx.snapshot_id, ctx.image_url, 200);
    ...
    get_instance(meta_service.get(), cloud_unique_id, instance_info);
    auto recycler = get_instance_recycler(meta_service.get(), instance_info);
    ASSERT_EQ(recycler->recycle_cluster_snapshots(), 0); // recycled
    ASSERT_EQ(recycler->recycle_cluster_snapshots(), 0); // pruned

    // The exceeds snapshot should be recycled.
    get_snapshots(meta_service.get(), instance_id, snapshots);
    ASSERT_EQ(snapshots.size(), 1);
    ASSERT_EQ(snapshots[0].status(), SnapshotStatus::SNAPSHOT_NORMAL);
}

// A test case that the referenced snapshot is not recyclable even if the max reserved snapshots is exceeded.
TEST(RecycleSnapshotTest, RecycleReferencedSnapshot) {
    auto meta_service = get_meta_service();
    auto txn_kv = meta_service->txn_kv();
    std::string instance_id = "recycle_snapshot_test_referenced_snapshot_instance";
    std::string cloud_unique_id = fmt::format("1:{}:0", instance_id);
    MOCK_GET_INSTANCE_ID(instance_id);
    create_and_refresh_instance(meta_service.get(), instance_id);
    int64_t db_id = 1, table_id = 2, index_id = 3, partition_id = 4, tablet_id = 5;
    ...
    begin_snapshot(meta_service.get(), cloud_unique_id, "auto-gen", &ctx);
    commit_snapshot(meta_service.get(), cloud_unique_id, ctx.snapshot_id, ctx.image_url, 120);

    {
        // Create a reference to the first snapshot.
        std::string data = unhex(snapshots[0].snapshot_id());
        ASSERT_EQ(data.size(), 10);
        std::array<uint8_t, 10> buffer;
        std::copy(data.begin(), data.end(), buffer.begin());
        Versionstamp v = Versionstamp(buffer);

        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
        std::string key = versioned::snapshot_reference_key({instance_id, v, "another_instance"});
        txn->put(key, "");
        ASSERT_EQ(txn->commit(), TxnErrorCode::TXN_OK);
    }
    ...
    // The referenced snapshot should not be recycled.
    get_snapshots(meta_service.get(), instance_id, snapshots);
    ASSERT_EQ(snapshots.size(), 2);
    ASSERT_EQ(snapshots[0].status(), SnapshotStatus::SNAPSHOT_NORMAL);
    ASSERT_EQ(snapshots[1].status(), SnapshotStatus::SNAPSHOT_NORMAL);
}

// A test case to recycle the manual snapshot if it is expired.
TEST(RecycleSnapshotTest, RecycleManualSnapshot) {
    auto meta_service = get_meta_service();
    auto txn_kv = meta_service->txn_kv();
    ...
    // begin a snapshot and abort it
    SnapshotContext ctx;
    begin_snapshot(meta_service.get(), cloud_unique_id, "manual-gen", &ctx, false);
    commit_snapshot(meta_service.get(), cloud_unique_id, ctx.snapshot_id, ctx.image_url, 100);
    ...
    ASSERT_EQ(snapshots[0].status(), SnapshotStatus::SNAPSHOT_NORMAL);
    ASSERT_EQ(snapshots[0].journal_id(), 100);
    ASSERT_EQ(snapshots[0].auto_snapshot(), false);
    ...
    // The manual snapshot should not be recycled.
    get_snapshots(meta_service.get(), instance_id, snapshots);
    ASSERT_EQ(snapshots.size(), 1);
    ASSERT_EQ(snapshots[0].status(), SnapshotStatus::SNAPSHOT_NORMAL);
    ASSERT_EQ(snapshots[0].journal_id(), 100);
