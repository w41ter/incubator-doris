...
public class InsertUtils {
     ...
     */
    public static String getFinalErrorMsg(String msg, String url) {
        int maxTotalBytes = 512;

        // For test
        // 1. error msg length > 512, we will truncate it first to make sure the URL
        //    keep complete.
        // 2. if the URL length > 512 too, we will remind user use
        //    `show load` to see detail
        ...
        // special case: url length > 512
        if (urlPartLen > maxTotalBytes) {
            useUrlPlaceholder = true;
            urlPartLen = ". url: please use `show load` for detail msg".length();
        }

        int maxMessageBytes = maxTotalBytes - urlPartLen;
