...
suite("test_clone_basic", "snapshot,docker") {
    // ATTN: This test only runs in cloud mode.
    if (!isCloudMode()) {
        logger.info("Skip test_clone_basic because not in cloud mode")
        return
    }
    ...
    def cluster_prefix = "regression_test_clone_basic_"
    ...
    dockers(["${base_name}": base_opt, "${derived_name}": derive_opt], manual_init_clusters) { clusters ->
        ...
        def cluster_snapshot_content = """
        {
            ...
            "obj_info": {
                ...
                "prefix": "regression_test_clone_basic",
