...
suite("test_multi_version_dup_table_index_operation", "snapshot,docker") {
    if (!isCloudMode()) {
        logger.info("Skip test_multi_version_dup_table_index_operation because not in cloud mode")
        return
    }

    def create_dup_table = { tableName ->
    ...
    def test_combination = { host, indexType, operation, tableNum ->
        def tableName = "dup_table_${indexType.toLowerCase()}_${operation.toLowerCase()}_${tableNum}"
        
        logger.info("Testing DUP table with index=${indexType}, operation=${operation}")
        
        create_dup_table(tableName)
    ...
    def cluster_name = "regression_test_dup_multi_version_table_index_operation"
