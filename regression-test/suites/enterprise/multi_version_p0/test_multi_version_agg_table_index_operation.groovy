...
suite("test_multi_version_agg_table_index_operation", "snapshot,docker") {
    if (!isCloudMode()) {
        logger.info("Skip test_multi_version_agg_table_index_operation because not in cloud mode")
        return
    }

    def create_agg_table = { tableName ->
    ...
    // only insert and delete operations are supported for AGG table
    def operations = [
    ...
    def set_multi_version_status = { host, status ->
        ...
        def (code, out, err) = curl('POST', url, null, 180)
    ...
    def test_combination = { host, indexType, operation, tableNum ->
        def tableName = "agg_table_${indexType.toLowerCase()}_${operation.toLowerCase()}_${tableNum}"
        
        logger.info("Testing AGG table with index=${indexType}, operation=${operation}")
        
        create_agg_table(tableName)
        
        if (indexType == "NGBF") {
            index_types[indexType](tableName, "k3")
            wait_alter_table_column_finished(tableName)
        } else {
            index_types[indexType](tableName, "k1")
            wait_alter_table_column_finished(tableName)
        }
        ...
        // DISABLED -> WRITE_ONLY -> READ_WRITE -> DISABLED -> WRITE_ONLY -> READ_WRITE -> DISABLED
    ...
    def cluster_name = "regression_test_agg_multi_version_table_index_operation"
